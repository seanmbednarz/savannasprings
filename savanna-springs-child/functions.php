...
/* Salt-delivery lead form handler. */
function ss_handle_salt_delivery() {
	if ( ! isset( $_POST['ss_salt_nonce'] ) || ! wp_verify_nonce( $_POST['ss_salt_nonce'], 'ss_salt' ) ) {
		wp_safe_redirect( home_url( '/' ) );
		exit;
	}
	$fields = array( 'first_name', 'last_name', 'phone', 'email', 'address', 'city', 'state', 'zip', 'bags', 'account' );
	$data   = array();
	foreach ( $fields as $f ) { $data[ $f ] = isset( $_POST[ $f ] ) ? sanitize_text_field( wp_unslash( $_POST[ $f ] ) ) : ''; }
	$data['notes'] = isset( $_POST['notes'] ) ? sanitize_textarea_field( wp_unslash( $_POST['notes'] ) ) : '';
	...
	$lead_id = wp_insert_post( array(
		'post_type'   => 'ss_lead',
		'post_title'  => sprintf( 'Salt — %s — %s', $name ?: 'Lead', $data['zip'] ),
		'post_status' => 'private',
	) );
	if ( $lead_id && ! is_wp_error( $lead_id ) ) {
		update_post_meta( $lead_id, 'ss_lead_type', 'Salt delivery' );
		foreach ( $data as $k => $v ) { update_post_meta( $lead_id, 'ss_' . $k, $v ); }
	}

	$to   = get_option( 'admin_email' );
	$body = "New salt delivery request:\n\n";
	foreach ( $data as $k => $v ) { $body .= ucwords( str_replace( '_', ' ', $k ) ) . ": $v\n"; }
	wp_mail( $to, 'New Salt Delivery request', $body );
